...
class index {
	...
	public function init() {
		...
		if(isMobile()) $default_style = 'mobile';
	...
	public function search(){
		...
		if($siteid == '1'){
			$table_news = 'phpcms_news';
			// $table_donghua = 'phpcms_video';
		} else {
			$table_news = 'phpcms_news_en';
			$table_donghua = 'phpcms_video_en';
		}
		...
		if(!empty($keyword)){
			// 联合表
			// $union_table = "select title, url, inputtime from {$table_news} where title like '%{$keyword}%' UNION select title, url, inputtime from {$table_donghua} where title like '%{$keyword}%'";
			// 查询所有结果
			$sql = "select title, url, inputtime from $table_news where title like '%{$keyword}%' order by inputtime desc";
			$result = $this->db->query($sql);
			$alldata = $this->db->fetch_array();
			$count = count($alldata);

			// 分页查询
			$limit = 10;
			$start_limit = ($page-1)*$limit;
			$sql = "select title, url, inputtime from $table_news where title like '%{$keyword}%' order by inputtime desc limit {$start_limit}, {$limit}";

			$result = $this->db->query($sql);
			$lists = $this->db->fetch_array();

			$pages = pages($count, $page, $limit);
		} else {
			$lists = '';
			$count = 0;
			$pages = '';
		...
		include template($contentPath,'searchlist',$default_style);
	}
	//内容页
	...
	public function preview(){
		...
		if($type == 1){
			...
			$sql = "select preview from phpcms_report_data where id = $id";
			$db = $report_model->query($sql);	
			$row = $db->fetch_array();
			$filepath = rtrim($row['preview'], '|');
		}
		$uploadpath = pc_base::load_config('system','upload_path');
