...
UploadImg.prototype.init = function() {
	var html = '<input type="file" id="'+this.options.file_name+'" name="'+this.options.file_name+'" class="fileInput'+this.options.id+'">'
				+'<div><img class="selectbtn uploadBtn'+this.options.id+'" src="/statics/default/images/up.jpg" width="202" height="120"></div>'
				+'<div class="previewBox'+this.options.id+'"></div>';
...
UploadImg.prototype.bindEvent = function() {
	var self = this;
	this.$obj.on("click.choose",".uploadBtn"+self.options.id,function() {
		$(".fileInput"+self.options.id).trigger("click");
	});
	$(".fileInput"+self.options.id).on("change.upload",function() {
...
UploadImg.prototype.addImgHtml = function(url) {
	if(this.options.picNum == 1) {
		$('.uploadBtn'+this.options.id).attr('src', url);
		// if($(".previewBox"+this.options.id)) {
		// 	$(".previewBox"+this.options.id).html("<img src="+ url +" width="+ this.options.width +" height="+ this.options.height +">");
		// }
	} else {
		if($(".previewBox"+this.options.id) && this.num < this.options.picNum) {
			$(".previewBox"+this.options.id).append("<img src="+ url +" width="+ this.options.width +" height="+ this.options.height +">");
			this.num++;
		}	
	}
},
...
UploadImg.prototype.previewImgIE = function(obj) {
	...
	if(document.selection) {
		...
		if(this.isImg(url)) {
			...
			if(this.options.picNum == 1) {
				$(".previewBox"+this.options.id).html($(imgWrap));
			} else if(this.options.picNum > 1 && this.num < this.options.picNum) {
				$(".previewBox"+this.options.id).append($(imgWrap));
				this.num++;
			} else {
				return;
			};
